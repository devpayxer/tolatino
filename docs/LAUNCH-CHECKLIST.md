...
- [ ] **🟠 La sala de juegos vive SOLO en pruebas — cinco migraciones sin
  ...
  de espera), `0160` (reglas del dominó), `0161` (reloj de 60 s y tiempo real),
  `0162` (identidad del rival en la mesa) y `0163` (jugada automática al vencer
  el tiempo) están ejecutadas en **pruebas**
  ...
  · la `0161` hace `alter publication supabase_realtime add table
    ...
    parece congelado;
  · la `0162` y la `0163` reemplazan `domino_estado` (y la `0163` además
    `domino_jugar`, `domino_robar`, `domino_pasar` y `domino_vencer`), así que
    si producción divergiera, `create or replace` escribiría encima de otra
    cosa: comparar antes.
