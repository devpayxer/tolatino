-- To'Latino — "Feriados y más": date-specific hour overrides on top of the weekly
-- schedule (holidays closed / special hours, vacations, weather, etc.). Stored as
-- a jsonb array on the business; the public listing's live status honors them.
-- Threads hours_exceptions through BOTH read paths. Return-type change → drop +
-- recreate. Idempotent. Apply: paste into the SQL Editor + Run.

alter table public.businesses add column if not exists hours_exceptions jsonb;

-- ── businesses_v2: geo feed + hours_exceptions ──────────────────────────────
...
-- ── business_by_slug: single listing by slug + hours_exceptions ─────────────
