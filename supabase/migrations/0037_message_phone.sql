-- To'Latino — let the owner use a DIFFERENT number for messages than the one for
-- calls (e.g. a WhatsApp mobile separate from the landline). Adds message_phone
-- (null = use the main phone) and threads it through BOTH read paths.
-- Return-type change → drop + recreate. Idempotent.
-- Apply: paste into the Supabase SQL Editor and Run.

alter table public.businesses add column if not exists message_phone text;

-- ── businesses_v2: geo feed + message_phone ─────────────────────────────────
...
-- ── business_by_slug: single listing by slug + message_phone ────────────────
