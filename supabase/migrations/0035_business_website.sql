-- To'Latino — add a Website field to a business listing. The dashboard's
-- "Información general" gets a Sitio web input (functional: persists + shows on
-- the public listing as a clickable link). Adds businesses.website and threads it
-- through BOTH read paths. Return-type change → drop + recreate. Idempotent.
-- Apply: paste into the Supabase SQL Editor and Run.

alter table public.businesses add column if not exists website text;

-- ── businesses_v2: geo feed + website ───────────────────────────────────────
...
-- ── business_by_slug: single listing by slug + website ──────────────────────
