-- To'Latino — business LOGO. Uploaded from Fotos y media (compressed to WebP
-- client-side like every image), stored as a URL on the business, and shown on
-- the dashboard header + the public search cards. Threads logo_url through BOTH
-- read paths. Return-type change → drop + recreate. Idempotent.
-- Apply: paste into the SQL Editor + Run.

alter table public.businesses add column if not exists logo_url text;

-- ── businesses_v2: geo feed + logo_url ──────────────────────────────────────
...
-- ── business_by_slug: single listing by slug + logo_url ─────────────────────
