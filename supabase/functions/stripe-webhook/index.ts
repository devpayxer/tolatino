  ...
  try {
    if (pending.kind === 'order') {
      ...
      const r = await rpc(url, service, 'fulfill_event_tickets_multi', {
        in_buyer: pending.buyer_id, in_slug: pending.ref, in_items: payload.items ?? [], in_promo: payload.promo ?? null, in_seats: payload.seats ?? null, in_addons: payload.addons ?? null,
      });
