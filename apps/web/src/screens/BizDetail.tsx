...
export function BizDetail({ b: bProp, all, onClose, onOpenOther }: { b: Business; all: Business[]; onClose: () => void; onOpenOther: (b: Business) => void }) {
  ...
  // Rental CART (party-rental model, 0097): pick several items for ONE event
  // date range → one order. rentCart maps item id → units. Day-based (the common
  ...
  const [rentDoneInfo, setRentDoneInfo] = useState<{ count: number; total: number; deposit: number; range: string; confirmedNow: boolean } | null>(null);
  ...
  useEffect(() => { if (!rentCartOpen) setRentPromo(null); }, [rentCartOpen]);
  ...
  // calendar for the whole cart. Fee = day-rate × days (weekly rate auto-applied
  // for 7+ day spans), plus a refundable deposit collected at pickup.
  const rentSpan = () => (rentStart ? (rentEnd ? spanDaysInc(rentStart, rentEnd) : 1) : 0);
  // fee for ONE unit over the chosen span (weekly rate auto-applied for 7+ days).
  const rentUnitFee = (it: PubRental) => {
    const n = rentSpan();
    if (it.week > 0 && n >= 7) { const w = Math.floor(n / 7), r = n % 7; return w * it.week + r * it.day; }
    return n * it.day;
  };
  // Cart lines resolved from the live item list (item + chosen units). Fee is the
  // per-unit fee over the shared date range × units; deposit is per-unit × units.
  const rentAutoConfirm = realRentals?.autoConfirm ?? false;
  const rentCartLines = rentalItems.map((it) => ({ it, units: rentCart[it.id] ?? 0 })).filter((l) => l.units > 0);
  const rentCartCount = rentCartLines.reduce((s, l) => s + l.units, 0);
  const rentLineFee = (it: PubRental, units: number) => rentUnitFee(it) * units;
  const rentLineDeposit = (it: PubRental, units: number) => it.dep * units;
  const rentExtrasChosen = (realRentals?.addons ?? []).filter((a) => rentExtras.includes(a.id));
  const rentExtrasFee = rentExtrasChosen.reduce((s, a) => s + a.price, 0);
  ...
  // Cart checkout — one rental ORDER (create_rental_order, 0097). Pay-at-pickup:
  // the whole order (all items for the chosen event dates) is settled at pickup,
  // where the refundable deposit is also collected. The server sets the order
  // status from the business's approval mode (auto → confirmed, manual → pending).
  const rangeLabel = () => {
 ...
 tab === k ? 'border-primary font-extrabold text-ink' : 'border-transparent font-bold text-muted-2'
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // RIEL DE ESCRITORIO — solo ≥1100px
  // ══════════════════════════════════════════════════════════════════════════
  // DE DÓNDE SALE: el fundador (2026-08-06) mandó un handoff nuevo porque «en
  // el escritorio se ve muy desolado». Tenía razón y la causa era de una línea:
  // toda la ficha vivía dentro de `max-w-[680px]`, así que en un monitor de
  // 1320 la mitad de la pantalla era fondo vacío. Es la ficha de móvil estirada.
  //
  // Esto NO reescribe la pantalla (el handoff pedía reconstruirla; ver la nota
  // en el commit): re-coloca en escritorio lo que ya existe, siguiendo su §11.
  // Todo lo que hay aquí sale de datos REALES que la ficha ya tiene — horario,
  // dirección, teléfono, promo del menú. Ningún bloque se inventa contenido:
  // el que no tiene dato, no se pinta.
  // La acción principal sale de lo que el negocio DE VERDAD ofrece (los mismos
  // tabs que ya se calcularon), no de una lista fija por categoría: si el dueño
  // no vende, no aparece un botón que no lleva a ningún sitio.
  const accion: { label: string; onClick: () => void } | null = (() => {
    // Un botón que lleva a donde ya estás no es una acción: es ruido. Si el
        ...
        : tabShown.services ? ['services', 'Reservar', 'Book']
          : tabShown.shop ? ['shop', 'Ver la tienda', 'Shop']
            : tabShown.rentals ? ['rentals', 'Ver la renta', 'View rentals']
              : tabShown.events ? ['events', 'Ver eventos', 'View events']
                : null;
    if (destino && tab !== destino[0]) {
      return { label: L(destino[1], destino[2]), onClick: () => onTab(destino[0]) };
    }
    if (msgOn) return { label: L('Enviar mensaje', 'Send message'), onClick: () => void openChat() };
    return null;
  })();
  ...
  const railCard = 'rounded-card border border-line bg-white p-4 ';
  const desktopRail = (
            ...
            <div className="flex flex-col gap-1.5">
              {WEEK.map((d, i) => {
                ...
                const label = isToday && todayExText != null
                  ? todayExText
                  : fmtDayHours(b.hours![hoursIdx], L('Cerrado', 'Closed'));
                return (
                  <div key={d[0]} className="flex items-center justify-between text-[12px] font-semibold">
                    <span className={isToday ? 'font-extrabold text-primary-dark' : 'text-muted'}>{B(d)}{isToday ? ` · ${L('Hoy', 'Today')}` : ''}</span>
                    <span className={isToday ? 'font-extrabold text-ink' : 'text-ink-2'}>{label}</span>
                  </div>
                );
              })}
            </div>
  ...
  const catalogCard = (it: MenuItem) => (
    <div key={B(it.n)} className="flex items-start gap-3 rounded-card-sm border border-line bg-white p-3">
  ...
  const itemCard = (catKey: string, it: MenuItem, displayOnly = false) => {
    ...
    return (
      <button
        ...
        className={`flex w-full items-center gap-3 rounded-card-sm border border-line bg-white p-3 text-left ${soldOut ? 'cursor-default opacity-75' : 'cursor-pointer'}`}
      >
        {itemBody(it)}
            ...
            <>
              ...
              {qty === 0 ? (
                <span
                  ...
                  className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-lilac text-primary-dark shadow-float transition-transform active:scale-90"
                >
                  <Plus size={16} stroke={2.8} />
                </span>
              ) : (
                <span className="flex items-center gap-0.5 rounded-full border border-lilac-line bg-white p-0.5 shadow-float">
                  <span
                    role="button"
  ...
  const gridCard = (catKey: string, it: MenuItem, displayOnly = false) => {
    ...
    return (
      <div
        key={`${catKey}:${B(it.n)}`}
        onClick={() => { if (!soldOut) openItem(catKey, it); }}
        className={`flex flex-col overflow-hidden rounded-card-sm border border-line bg-white transition-shadow hover:shadow-card-lg ${soldOut ? 'opacity-75' : 'cursor-pointer'}`}
      >
        <div className="relative aspect-[4/3] w-full" style={{ background: `repeating-linear-gradient(135deg,${it.bg})` }}>
          ...
          {off > 0 && !soldOut && (
            <span className="absolute left-2 top-2 rounded-md bg-pink px-1.5 py-0.5 text-[10px] font-extrabold text-white shadow-float">−{off}%</span>
          )}
          {!displayOnly && (soldOut ? (
            <span className="absolute bottom-2 right-2 rounded-full bg-white/95 px-2.5 py-1.5 text-[9.5px] font-extrabold text-muted-2 shadow-float">{L('Agotado', 'Sold out')}</span>
          ) : qty === 0 ? (
            <span
              role="button"
              aria-label={L('Agregar', 'Add')}
  ...
  const svcCard = (s: PubSvc) => {
    ...
    return (
      <div key={s.id} onClick={() => { setSvcAddOns({}); setSvcInfo(s); }} className="flex cursor-pointer items-start gap-3 rounded-card-sm border border-line bg-white p-3">
  ...
  return (
    // El ancho de siempre en móvil y tablet; a partir de 1100px la ficha se
    // abre a dos columnas. Antes TODO vivía en 680px: en un monitor de 1320 la
    // mitad de la pantalla era fondo vacío — el «se ve muy desolado» del
    // fundador (2026-08-06).
    <div className="mx-auto max-w-[680px] min-[1100px]:max-w-[1240px]">
      ...
      <div className="relative mb-4 h-[268px] overflow-hidden rounded-card min-[1100px]:mb-0 min-[1100px]:h-[420px]" style={{ background: bizTile(b) }}>
        ...
        <div className="absolute left-3 right-3 top-3 flex items-center justify-between">
          <button onClick={onClose} className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-white shadow-float min-[1100px]:invisible" aria-label={L('Volver', 'Back')}>
            <ChevronLeft size={18} stroke={2.4} className="text-ink" />
          </button>
          <div className="flex gap-2.5">
            <button onClick={() => setContactOpen(true)} className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-white shadow-float" aria-label={L('Compartir', 'Share')}>
              <Share size={16} stroke={2.2} className="text-ink" />
            </button>
            <button onClick={toggleSave} className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-white shadow-float" aria-label={L('Guardar', 'Save')}>
        ...
        {photos.length > 0 && (
          <>
            ...
            <button onClick={() => setGalIdx(0)} className="absolute bottom-4 right-4 hidden cursor-pointer items-center gap-1.5 rounded-[10px] bg-white px-3 py-[7px] text-[12px] font-extrabold text-ink shadow-float min-[1100px]:flex">
      ...
      {photos.length > 1 && (
        <div className="hidden min-[1100px]:grid min-[1100px]:h-[420px] min-[1100px]:grid-cols-2 min-[1100px]:grid-rows-2 min-[1100px]:gap-2">
          ...
          {Array.from({ length: Math.max(0, 4 - Math.min(photos.length - 1, 4)) }).map((_, i) => (
            <div key={`h${i}`} className="rounded-[15px]" style={{ background: bizTile(b) }} />
          ))}
        </div>
      )}
      </div>
 ...
 focused ? 'bg-canvas -mt-4 md:-mt-5 lg:-mt-[26px]' : '-mt-[46px]'
      ...
      >
        ...
        <div className="pointer-events-auto mx-5 hidden items-center gap-3 rounded-card border border-line bg-white px-4 py-2 min-[1100px]:flex">
        ...
        <div className="pt-5">
          ...
          {buenoSaber.length >= 2 && (
            <>
              ...
              <div data-bueno-saber className="grid grid-cols-2 gap-2.5 min-[1100px]:grid-cols-4">
                {buenoSaber.map((t) => (
                  <div key={t.label} className="rounded-tile border border-line bg-white p-3">
              ...
              <div className="no-scrollbar -mx-3.5 flex touch-pan-x gap-2.5 overflow-x-auto overscroll-x-contain px-3.5 pb-1 md:-mx-5 md:px-5 min-[1100px]:mx-0 min-[1100px]:grid min-[1100px]:grid-cols-4 min-[1100px]:overflow-visible min-[1100px]:px-0">
                {populares.map(({ catKey, it }) => (
                  <button key={`${catKey}-${it.n[0]}`} onClick={() => openItem(catKey, it)}
                    className="tap w-[152px] flex-none cursor-pointer overflow-hidden rounded-tile border border-line bg-white text-left min-[1100px]:w-auto">
          ...
          {(() => {
            ...
            const tarjeta = (r: (typeof rows)[number], i: number) => (
              <div key={i} className="rounded-card border border-line bg-white p-3.5">
                <div className="flex items-center gap-2.5">
            ...
            return (
              <>
                ...
                <div className="min-[1100px]:grid min-[1100px]:grid-cols-[220px_minmax(0,1fr)] min-[1100px]:gap-4">
                  ...
                  <div className="hidden rounded-card border border-line bg-white p-4 min-[1100px]:block">
              ...
              <div className="no-scrollbar -mx-3.5 flex touch-pan-x gap-2.5 overflow-x-auto overscroll-x-contain px-3.5 pb-1 md:-mx-5 md:px-5 min-[1100px]:mx-0 min-[1100px]:grid min-[1100px]:grid-cols-3 min-[1100px]:overflow-visible min-[1100px]:px-0">
                {similares.map((s) => (
                  <button key={s.slug} onClick={() => void openRelated(s.slug)}
                    className="tap w-[168px] flex-none cursor-pointer overflow-hidden rounded-tile border border-line bg-white text-left min-[1100px]:w-auto">
            ...
            <div style={{ top: menuStickyTop }} className="group sticky z-[15] -mx-3.5 mb-4 border-b border-line bg-canvas md:-mx-5">
              ...
              {railNav.left && (
                <>
                  ...
                  <button type="button" aria-label={L('Categorías anteriores', 'Previous categories')} onClick={() => railBy(-240)} className="absolute left-1.5 top-1/2 hidden h-8 w-8 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border border-line bg-white text-ink opacity-0 shadow-float transition-opacity hover:bg-lilac-2 group-hover:opacity-100 md:flex">
              ...
              {railNav.right && (
                <>
                  ...
                  <button type="button" aria-label={L('Más categorías', 'More categories')} onClick={() => railBy(240)} className="absolute right-1.5 top-1/2 hidden h-8 w-8 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border border-line bg-white text-ink opacity-0 shadow-float transition-opacity hover:bg-lilac-2 group-hover:opacity-100 md:flex">
                ...
                {!rentDisplayOnly && (
                  soldOut ? (
                    ...
                    <span className="flex flex-none items-center gap-2.5 rounded-full bg-lilac-2 px-2 py-1.5">
                      ...
                      <button onClick={() => addToCart(it.id, 1, cap)} disabled={inCart >= cap} aria-label={L('Agregar uno', 'Add one')} className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-full bg-white text-[16px] font-extrabold text-ink disabled:opacity-40">+</button>
                    </span>
                  ) : (
                    <button
                      onClick={() => addToCart(it.id, 1, cap)}
                ...
                <div className="no-scrollbar -mx-1 flex snap-x snap-mandatory gap-2 overflow-x-auto px-1">
                  {(gallery.length ? gallery : ['']).map((u, i) => (
                    <div key={i} className={`relative aspect-[4/3] flex-none snap-center overflow-hidden rounded-card ${gallery.length > 1 ? 'w-[86%]' : 'w-full'}`} style={{ background: `repeating-linear-gradient(135deg,${mIt.bg})` }}>
                      ...
                      {mOff > 0 && i === 0 && <span className="absolute left-2.5 top-2.5 rounded-md bg-pink px-2 py-0.5 text-[11px] font-extrabold text-white shadow-float">−{mOff}%</span>}
              ...
              <>
                ...
                <div className={`flex flex-col overflow-y-auto ${storeCart ? 'max-h-[300px] gap-3' : 'max-h-[240px] gap-2.5'}`}>
                  {Object.entries(cart).map(([k, l]) => {
                    ...
                    return (
                      <div key={k} className="rounded-card-sm border border-line bg-white p-3">
          ...
          return (
            <>
              <div className="relative -mx-4 -mt-4 h-[130px] overflow-hidden rounded-t-panel md:-mx-5 md:-mt-5 md:rounded-t-card" style={{ background: `repeating-linear-gradient(135deg,${s.tile})` }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                {s.img && <img src={imgUrl(s.img, ANCHO.tarjeta)} alt="" className="absolute inset-0 h-full w-full object-cover" />}
                <button onClick={() => setSvcInfo(null)} className="absolute left-3 top-3 flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-white shadow-float">
      ...
      <Overlay open={rentCartOpen} onClose={() => { setRentCartOpen(false); setRentDone(false); }} width={440}>
                ...
                {rentCartLines.map((l) => (
                  <div key={l.it.id} className="flex items-center gap-3 rounded-field border border-line p-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] font-bold text-muted">{money(l.it.day)}/{L('día', 'day')} · {L('dep.', 'dep.')} {money(l.it.dep)}</div>
                    </div>
                    <span className="flex flex-none items-center gap-2 rounded-full bg-lilac-2 px-1.5 py-1">
                      <button onClick={() => addToCart(l.it.id, -1, rentCartStock(l.it))} className="flex h-6 w-6 cursor-pointer items-center justify-center rounded-full bg-white text-[15px] font-extrabold text-ink">−</button>
                      <span className="w-4 text-center text-[13px] font-extrabold text-ink">{l.units}</span>
                      <button onClick={() => addToCart(l.it.id, 1, rentCartStock(l.it))} disabled={l.units >= rentCartStock(l.it)} className="flex h-6 w-6 cursor-pointer items-center justify-center rounded-full bg-white text-[15px] font-extrabold text-ink disabled:opacity-40">+</button>
                    </span>
                  </div>
                ))}
              </div>

              {/* order-level extras (delivery, setup…) */}
              {(realRentals?.addons.length ?? 0) > 0 && (
                <>
                  <div className="mb-2 mt-4 text-[13px] font-extrabold text-ink">{L('Extras del pedido', 'Order extras')}</div>
                  <div className="flex flex-col gap-2">
                    {(realRentals?.addons ?? []).map((a) => {
                      const on = rentExtras.includes(a.id);
                      return (
                        <button key={a.id} onClick={() => setRentExtras((l) => (on ? l.filter((x) => x !== a.id) : [...l, a.id]))} className={`flex items-center gap-3 rounded-field border-[1.5px] p-3 text-left ${on ? 'border-primary bg-lilac-3' : 'border-lilac-line bg-white'}`}>
        ...
        {rentCartOpen && rentDone && rentDoneInfo && (
          <div className="flex flex-col items-center px-2 py-6 text-center">
            <span className={`flex h-[72px] w-[72px] items-center justify-center rounded-full ${rentDoneInfo.confirmedNow ? 'bg-primary shadow-cta' : 'bg-green-bg'}`}>
              <Check size={30} stroke={3} className={rentDoneInfo.confirmedNow ? 'text-white' : 'text-green'} />
            </span>
            <div className="mt-4 text-[21px] font-extrabold text-ink">{rentDoneInfo.confirmedNow ? L('¡Renta confirmada!', 'Rental confirmed!') : L('¡Renta solicitada!', 'Rental requested!')}</div>
            <div className="mt-1.5 max-w-[300px] text-[13px] font-semibold leading-relaxed text-muted">
              {rentDoneInfo.confirmedNow
            ...
            <div className="mt-4 w-full max-w-[300px] rounded-card-sm border border-line bg-white p-4 text-left">
              ...
              <div className="mt-2 flex items-center justify-between border-t border-hair pt-2">
                <span className="text-[12.5px] font-bold text-muted">{rentDoneInfo.count} {rentDoneInfo.count === 1 ? L('artículo', 'item') : L('artículos', 'items')}</span>
                <span className="text-[13px] font-extrabold text-primary-dark">{money(rentDoneInfo.total)}</span>
              </div>
              <div className="mt-2 flex items-center justify-between border-t border-hair pt-2">
                <span className="text-[12px] font-bold text-muted">{L('Depósito reembolsable', 'Refundable deposit')}</span>
                <span className="text-[12.5px] font-extrabold text-ink">{money(rentDoneInfo.deposit)}</span>
              </div>
            </div>
            <PrimaryBtn className="mt-5" onClick={() => { setRentCartOpen(false); setRentDone(false); }}>
              {L('Listo', 'Done')}
            </PrimaryBtn>
          </div>
        )}
