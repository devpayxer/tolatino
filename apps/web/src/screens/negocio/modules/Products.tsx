...
// Productos / Envío module (business dashboard). Two modes via a top toggle:
//   · Productos → sell-online vs catalog-only switch + sub-tabs
//     Catálogo · Inventario · Variantes · Colecciones · Descuentos.
//   · Envío → sub-tabs Zonas · Recoger · Envío nacional · Repartidores.
// Includes a 4-step "add product" wizard (Detalles → Precio → Inventario →
// Revisar) with a live preview + success screen, an edit bottom-sheet, and
...
type ShipTab = 'zones' | 'pickup' | 'ship' | 'drivers';
...
export function ProductsModule({ ctx, tab }: { ctx: PanelCtx; tab: TabKey }) {
  ...
  const [shipTab, setShipTab] = useState<ShipTab>('zones');
  ...
  const goMode = (m: 'products' | 'shipping') => { setMode(m); if (m === 'products') setProdTab('catalog'); else setShipTab('zones'); };
  ...
  // ---------- shipping · pickup ----------
  const pickupLocs = [
    { name: '5821 Bellaire Blvd', main: true, hours: L('7 AM – 8 PM diario', '7 AM – 8 PM daily'), window: L('listo en 30 min', 'ready 30 min') },
  ...
  const extRaw = [
    ...
    { label: 'G', color: '#FF8000', name: 'GrubHub Direct', dEs: 'Cobertura extra en noches ocupadas.', dEn: 'Extra coverage on busy nights.', rate: '—', cEs: 'Conectar', cEn: 'Connect' },
  ];
  ...
  const shipSub: [ShipTab, string][] = [
    ['zones', L('Zonas', 'Zones')],
    ['pickup', L('Recoger', 'Pickup')],
    ['ship', L('Envío nacional', 'Ship anywhere')],
    ['drivers', L('Repartidores', 'Drivers')],
  ];
        ...
        : prodTab === 'inventory' ? inventoryView
          : prodTab === 'variants' ? variantsView
            : prodTab === 'collections' ? collectionsView
              : discountsView
      : shipTab === 'zones' ? zonesView
        : shipTab === 'pickup' ? pickupView
          : shipTab === 'ship' ? shipAnyView
            : driversView;
  ...
  const railLinks = (rows: { Icon: typeof Route; cls: string; title: string; sub: string; right?: string; onClick: () => void }[]) => (
    <div className={`${cardCls} p-4`}>
      <div className="mb-3 text-[13px] font-extrabold text-ink">{mode === 'products' ? L('Envío y entrega', 'Shipping & delivery') : L('Atajos', 'Shortcuts')}</div>
      <div className="flex flex-col gap-2.5">
        {rows.map((r) => (
  ...
  const rail = (
    <div className="flex flex-col gap-4 xl:sticky xl:top-[74px] xl:self-start">
      <div className={`${cardCls} p-4`}>
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {(mode === 'products'
            ...
            : [[L('Zonas', 'Zones'), '3'], [L('Entregas hoy', 'Deliveries'), '42'], [L('Tiempo prom.', 'Avg time'), '18m'], [L('Repartidores', 'Drivers'), '3/5']]
          ).map(([l, v]) => (
      ...
      {mode === 'products'
        ? railLinks([
            { Icon: MapPin, cls: 'bg-green-bg text-green-dark', title: L('Entrega local', 'Local delivery'), sub: L('3 zonas · desde $5', '3 zones · from $5'), onClick: () => { goMode('shipping'); setShipTab('zones'); } },
            { Icon: Truck, cls: 'bg-blue-bg text-blue', title: L('Envío nacional', 'National shipping'), sub: 'USPS · UPS · FedEx', onClick: () => { goMode('shipping'); setShipTab('ship'); } },
            { Icon: HardHat, cls: 'bg-amber-bg text-amber-ink', title: L('Repartidores', 'Drivers'), sub: L('Propios o apps', 'Own or apps'), right: '3', onClick: () => { goMode('shipping'); setShipTab('drivers'); } },
          ])
        : railLinks([
            { Icon: Package, cls: 'bg-lilac text-primary-dark', title: L('Catálogo', 'Catalog'), sub: L('7 productos', '7 products'), onClick: () => goMode('products') },
            { Icon: Boxes, cls: 'bg-amber-bg text-amber-ink', title: L('Inventario', 'Inventory'), sub: L('2 por reabastecer', '2 to reorder'), right: '⚠', onClick: () => { goMode('products'); setProdTab('inventory'); } },
          ])}
    </div>
  );
    ...
    <div className="relative pb-8">
      ...
      <div className="mb-3 grid grid-cols-2 gap-2">
        <button onClick={() => goMode('products')} className={`flex items-center justify-center gap-2 rounded-btn py-2.5 text-[12.5px] font-extrabold ${mode === 'products' ? 'bg-ink text-white' : 'bg-lilac-2 text-ink-2'}`}>
          <Package size={15} strokeWidth={2} />{L('Productos', 'Products')}
        </button>
        <button onClick={() => goMode('shipping')} className={`flex items-center justify-center gap-2 rounded-btn py-2.5 text-[12.5px] font-extrabold ${mode === 'shipping' ? 'bg-ink text-white' : 'bg-lilac-2 text-ink-2'}`}>
          <Truck size={15} strokeWidth={2} />{L('Envío', 'Shipping')}
        </button>
      </div>
