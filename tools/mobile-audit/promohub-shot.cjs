...
(async()=>{const br=await chromium.launch({executablePath:'/opt/pw-browsers/chromium'});const p=await br.newPage({viewport:{width:390,height:1500}});p.on('pageerror',()=>{});
...
console.log('hub title:', await p.getByText('Nueva campaña').first().isVisible().catch(()=>false));
await p.screenshot({path:SCR+'/promohub.png',fullPage:true});
await br.close();})();
